...
class MessageStoreSpec extends Specification {
  ...
  val message3 = Message("user3@test.com", "user4@test.com", "My message to user4.")
  val message1Copy = message1.copy(messageID = Some(1))
  val message2Copy = message2.copy(messageID = Some(2))
  val message3Copy = message3.copy(messageID = Some(3))
  ...
  val InsertMessage = BeforeHook {
    DB.withConnection { implicit conn =>
      ...
      messageStore.insert(message3).toInt must beEqualTo(3)
    }
  }
  ...
  "Message Store" should {
    ...
    "not find a non-existent message by id" in InsertMessage {
      messageStore.getById(4) must beNone
    }
    ...
    "find root messages by email for sender" in InsertMessageReply {
      messageStore.getByEmail(message1.fromUser) must beEqualTo(List(message1ReplyCopy))
    }
    "find root messages by email for recipient" in InsertMessageReply {
      messageStore.getByEmail(message2.fromUser) must beEqualTo(List(message1ReplyCopy))
    }
    "get conversation by root message id" in InsertMessageReply {
      messageStore.getConversationById(1) must beEqualTo(List(message1ReplyCopy, message2ReplyCopy))
    }   
    "get conversation by internal message id" in InsertMessageReply {
      messageStore.getConversationById(2) must beEqualTo(List(message2ReplyCopy))
    }  
